...
class Lines(Board):
        ...
        if color in [0, 2]:
            if not self.has_red:
                ...
                if self.has_path(cell_xy[0], cell_xy[1]):
                    ...
                    arr = self.way(*cell_xy)
                    if arr:
                        self.move(arr)
                    # to_move = self.way(*cell_xy)
                    # print(to_move)
                    self.has_red = False
                    self.red_coords = []
                    self.visited = {}
    ...
    def move(self, arr):
        for first, second in arr:
            x, y, x2, y2, color = self.arr[first][second]
            circle(screen, 'red', (x + ((x2 - x) // 2), (y + ((y2 - y) // 2))), 22)
            pygame.display.update()  # Обновление экрана
            pygame.time.delay(50)  # Пауза в 2 секунды
            circle(screen, 'black', (x + ((x2 - x) // 2), (y + ((y2 - y) // 2))), 22)
        ...
        if self.need_to_continue:
            ...
            for x_2, y_2 in need:
                if (x_2, y_2) not in self.visited:
                    self.visited.get((x, y)).append((x_2, y_2))
                if [x_2, y_2] == list(self.red_coords[-1]):
    ...
    def way(self, x, y):
        start_parent, start_child = None, None
        now_el = None
        visited_2 = list(list(i) for i in self.visited.items())
        need = []
        while True:
            if not start_child:
                start_parent, start_child = visited_2.pop(0)
            else:
                if not now_el:
                    now_el = start_child[0]
                if now_el == (x, y):
                    break
                need.append(now_el)
                now_el = self.visited[now_el][0]
        return need
